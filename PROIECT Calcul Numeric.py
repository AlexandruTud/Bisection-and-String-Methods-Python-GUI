...
def calculate(): 
    ...
    if selected_method.get() == "bisection":
        ...
        result = coarda_er(fx, a, b, iterations)
        # Afișați rezultatele în câmpurile de text din panoul "Rezultate"
        textbox_xn.configure(state="normal")
        textbox_xn.delete(0, "end")
        textbox_xn.insert(0, result)
        ...
        result_coarda = coarda_er(fx, a, b, iterations)
...
labels = [label_a, label_b, label_fx, label_iterations, label_xn, label_error, label_method_wait, label_fzero_wait, method_label, graph_label]
for label in labels:
